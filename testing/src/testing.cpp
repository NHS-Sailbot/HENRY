...
constexpr static const char *PORT_ID[PORT_COUNT] = {
...
int open_port(unsigned int port_num) {
    file_id[port_num] = open(PORT_ID[port_num], O_RDWR | O_NOCTTY | O_NDELAY);
    if (file_id[port_num] == -1)
        return UNABLE_TO_OPEN_PORT;
    if (flock(file_id[port_num], LOCK_EX | LOCK_NB)) {
        close(file_id[port_num]);
        return SPECIFIED_PORT_IS_LOCKED;
    }
    int error = tcgetattr(file_id[port_num], old_port_settings + port_num);
    if (error == -1) {
        close(file_id[port_num]);
        flock(file_id[port_num], LOCK_UN);
    ...
    error = tcsetattr(file_id[port_num], TCSANOW, &port_settings);
    if (error == -1) {
        tcsetattr(file_id[port_num], TCSANOW, old_port_settings + port_num);
        close(file_id[port_num]);
        flock(file_id[port_num], LOCK_UN);
        return UNABLE_TO_SET_PORT_SETTINGS;
    }
    int status;
    if (ioctl(file_id[port_num], TIOCMGET, &status) == -1) {
        tcsetattr(file_id[port_num], TCSANOW, old_port_settings + port_num);
        flock(file_id[port_num], LOCK_UN);
    ...
    if (ioctl(file_id[port_num], TIOCMSET, &status) == -1) {
        tcsetattr(file_id[port_num], TCSANOW, old_port_settings + port_num);
        flock(file_id[port_num], LOCK_UN);
...
void handle_open_port(unsigned int port_num) {
    switch (open_port(port_num)) {
    case UNABLE_TO_OPEN_PORT:
        printf("unable to open port %d (%s)\n", port_num, PORT_ID[port_num]);
        break;
